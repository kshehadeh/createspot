...
import {
  dashboardPortfolioStripImageSizes,
  dashboardPortfolioStripScrollClass,
...
export function PortfolioSection({ portfolioUrl }: PortfolioSectionProps) {
  ...
  return (
    <DashboardSection title={t("title")} action={action} editorial>
      {loading ? (
        <div className={dashboardPortfolioStripScrollClass}>
          {Array.from({ length: 6 }).map((_, i) => (
            <Skeleton key={i} className="h-44 w-28 shrink-0 rounded-xl" />
          ))}
        </div>
              ...
              <HoverCard key={s.id} openDelay={200} closeDelay={100}>
                <HoverCardTrigger asChild>
                  <Link
                    ...
                    className={`group relative h-44 w-28 shrink-0 overflow-hidden bg-surface-lowest outline-none ${
                      s.isWorkInProgress
                        ? "rounded-xl border border-dashed border-outline-variant/40"
                        : "rounded-xl"
                    }`}
                  >
                    {displayImageUrl ? (
                      <Image
                        ...
                        sizes={dashboardPortfolioStripImageSizes}
                        style={{
